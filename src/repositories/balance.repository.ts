...
export default class BalanceRepository {
    static async updateBalance(client:PoolClient, userId: string, amount: number): Promise<number> {
        ...
        if (balance.rows.length === 0) {
            const newBalance = await pool.query('INSERT INTO wallets (id, user_id, balance) VALUES ($1, $2, $3) returning balance', [uuid(), userId, amount]);
            return newBalance.rows[0].balance;
        } else {
            const newBalance = await pool.query('UPDATE wallets SET balance = $1 WHERE user_id = $2 returning balance', [balance.rows[0].balance + amount, userId]);
            return newBalance.rows[0].balance;
        }
    }
